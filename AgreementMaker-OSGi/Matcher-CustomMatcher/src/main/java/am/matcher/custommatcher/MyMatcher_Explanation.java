...
public class MyMatcher_Explanation extends AbstractMatcher {
...
//		Phase 3
    	...
    	for(Mapping m:alignmentMappings) {
    		try {
	    		if(m.getEntity1().isClass() && m.getEntity2().isClass()) {
	    			ExplanationNode.generateExplanation=true;
	    			
						alignTwoNodes(m.getEntity1(), m.getEntity2(), alignType.aligningClasses, getClassesMatrix());
	    			...
	    			ExplanationNode.generateExplanation=true;
						...
						alignTwoNodes(m.getEntity1(), m.getEntity2(), alignType.aligningProperties, getPropertiesMatrix());
    ...
    protected void afterAlignOperations() {
    	...
    	System.out.println("So it comes here");
    }
