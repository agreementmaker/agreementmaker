...
public class Normalizer {
	...
	public String normalize(String s) {
		...
		for(int i = 0 ; i < chars.length; i++) {	
			...
			if(param.normalizeBlank && Character.isUpperCase(chars[i]))
				endOfWord = true;
			...
			if(endOfWord) {
				if(currentWord.length()>0 && !currentWord.equals(" ")) {
					if((!param.removeStopWords && !param.removeAllStopWords) || !stopWords.contains(currentWord)) { // if we pass this test it means that the word has to be added to the final processed string
						if(param.stem) {
							try {
    ...
    private void initNormalizeMap() {
		normalizeMap = new HashMap<>();
		//char to be always replaced
		String emptyString = "";
		String blank = " ";
		
		if(param.normalizeBlank) {
		...
		if(param.normalizeSlashes) {
			normalizeMap.put("/", blank);
			normalizeMap.put("\\", blank);
		}
		
		if(param.normalizeDigit) {
...
//			stopWords.add("on"); // this is also risky for on/off meaning like onCampus offCampus
		...
		if(param.removeAllStopWords) {
			stopWords = new HashSet<>();
            Collections.addAll(stopWords, EnglishUtility.stopwords);
		}
	}
