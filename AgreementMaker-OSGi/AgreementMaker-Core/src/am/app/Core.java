...
/**
 * SINGLETON JAVA PATTERN There will be only one instance of this class in the
 * whole application All classes can access it without having any reference to
 * it, just invoking the static method Core.getInstance() All model information
 * of the system will be accessible from this class: ontologies, matchings and
 * so on.
 * 
 * TODO: I think this class should be removed. It's a crutch and a better design
 * is needed. -- Cosmin, Oct. 20, 2013.
 * 
 */
public class Core {
	...
	// Program wide DEBUG flag. -- Deprecated, these flags will be removed and
	// replaced with log4j!!! -- Cosmin Aug. 3, 2012.
	@Deprecated
	public static final boolean DEBUG = false;
	@Deprecated
	public static final boolean DEBUG_STACK_TRACE_MSG = false;
	@Deprecated
	public static final boolean DEBUG_NORMALIZER = false; // debug flag for the
															// am.app.mappingEngine.StringUtil.Normalizer
															// class
	@Deprecated
	...
	/**
	 * Do not change this string without updating the AM_ROOT
	 * directory structure. 
	 */
	...
	/**
	 * List of matchers instances run by the user Data of the tableModel of the
	 * matcherTable is taken from this structure Can't be null;
	 */
	...
	/**
	 * Keep a list of completed matching tasks currently in the system. When two
	 * ontologies are to be aligned a matching task is created and then
	 * executed. The completed matching task is stored in this list, to be used
	 * by the various components in AgreementMaker (e.g., visualization).
	 */
	private final List<MatchingTask> completedMatchingTasks = new ArrayList<MatchingTask>();

	private int IDcounter = 0; // used in generating IDs for the ontologies and
								// matchers
	...
	/**
	 * The ArrayList of ontologies that are currently loaded into the
	 * AgreementMaker. This adds support for more than two ontologies to be
	 * loaded into the AgreementMaker
	 * 
	 * Also, when an ontology is loaded, we need to let the GUI know, so we have
	 * an OntologyChangeListener interface. Interested parts of our code can
	 * register as an OntologyChangeListener of the core, by calling: -
	 * addChangeListener() - or removeChangeListener() to remove from the
	 * listener list
	 */
	...
	private static LexiconStore lexstore; // The Lexicon store for these
											// ontologies.

	// private static UI ui; // A reference to the userinterface instance,
	// canvas and table can be accessed anytime.
	...
	/**
	 * It's private because it's not possible to create new instances of this
	 * class
	 */
	private Core() {

		// System.setProperty("log4j.debug","strue" ); // Use this to see what
		// log4j gets configured to.
		...
		if (amRoot == null) {
			...
			// First, check if AM_ROOT is a sibling project in the Eclipse
			// Workspace, which is a typical setup
			if (!checkRootDirectory("../AM_ROOT")) {
				// Second, check if AM_ROOT is a directory in the current
				// working directory.
				if (!checkRootDirectory("AM_ROOT")) {
					...
					amRoot = System.getProperty("user.dir",
		...
		loadedOntologies = new ArrayList<Ontology>(); // initialize the
														// arraylist of
														// ontologies.
		ontologyListeners = new ArrayList<OntologyChangeListener>(); // new list
																		// of
																		// listeners
		matcherListeners = new ArrayList<MatcherChangeListener>(); // another
																	// list of
																	// listeners
		// visualizationListeners = new
		// ArrayList<VisualizationChangeListener>();
	 ...
	 * 
	 * @param relativePath
	 *            The path to the AM_ROOT directory
	...
	private boolean checkRootDirectory(String relativePath) {
		...
		if (dir.exists()) {
			if (!dir.isDirectory()) {
				log.warn("Cannot use directory for AM_ROOT because it is a file: "
						+ dir.getAbsolutePath());
				return false;
			} else if (!dir.canRead()) {
				log.warn("Cannot use directory for AM_ROOT because it does not have read permissions: "
						+ dir.getAbsolutePath());
				return false;
			} else {
	...
	// multiple-ontology interface. - Cosmin 10/17/2010)
	...
	} // deprecated by multi-ontology interface

	public void setSourceOntology(Ontology sourceOntology) { // deprecated by
																// multi-ontology
																// array
	...
	public void setTargetOntology(Ontology targetOntology) { // deprecated by
																// multi-ontology
																// interface
	...
	/**
	 * This function will return the first Matcher instance of type "matcher" in
	 * the AM (ordered by its index).
	 * 
	 * @return Returns null if a matcher of the specified type does not exist in
	 *         the system.
	 * 
	 * @deprecated Use {@link #getMatchingAlgorithm(String)} or
	 *             {@link #getMatchingAlgorithms()}
	 */
	@Deprecated
	...
	/**
	 * <p>
	 * Returns a list of all the matchers currently registered in the system.
	 * </p>
	 * 
	 * <p>
	 * Currently this is implemented to work with the OSGi registry, iterate
	 * through all the registered matching algorithms, and return them as a
	 * list.
	 * </p>
	 * 
	 ...
	 * 
	 * @param matcherName
	 *            The (long) name of the matching algorithm.
	 * @return The first algorithm whose name is exactly the same as name. null
	...
	public AbstractMatcher getMatchingAlgorithm(String matcherName) {
		...
		for (AbstractMatcher matcher : matchers) {
			if (matcher.getName().equals(matcherName))
				return matcher;
		}
	...
	@Deprecated
	public List<AbstractMatcher> getMatcherInstances() {
		return getMatchingAlgorithms();
	}

	@Deprecated
	public List<MatcherResult> getMatcherResults() {
		return matcherResults;
	}

	public List<MatchingTask> getMatchingTasks() {
		return completedMatchingTasks;
	}
	...
	/**
	 * @return Return a result from the given result ID. Returns null if no
	 *         results match the id.
	 */
	@Deprecated
	...
	/**
	 * Remove the reference to a matching task. This is usually done when the
	 * matching task is removed the from matchers control panel.
	 * 
	 * @param mt
	...
	public static LexiconStore getLexiconStore() {
		return lexstore;
	}
	...
	/**
	 * Some selection parameters or some information in the alignMatrix of the
	 * matcher a are changed, so it's needed to invoke a.select() and also to
	 * update all other matchers with a as inputMatcher matchers are kept in
	 * chronological order in the list , so any matcher with a as input must be
	 * in a next entry of the list at the beginning there will be only one
	 * matcher in the modifiedList that is a anytime we find another matcher a2
	 * with a as input we have to invoke a2.match() and we have to add a2 to the
	 * modifiedmatchings so that all other matchers with a and/or a2 as input
	 * will be updated to and so on.
	 * 
	 * @param a
	 *            the matcher that has been modified and generates a chain
	 *            reaction on other matchings
	 * @throws AMException
	 */
	public void selectAndUpdateMatchers(MatchingTask t) throws Exception {
		t.select();
		MatchingTaskChangeEvent evt = new MatchingTaskChangeEvent(t,
				MatchingTaskChangeEvent.EventType.MATCHER_ALIGNMENTSET_UPDATED);
		fireEvent(evt);
		updateMatchers(t);
	}
	...
	/**
	 * add or update the alignments selected by the user in all the matchers
	...
	public void performUserMatching(int index, ArrayList<Mapping> alignments)
		...
		MatchingTaskChangeEvent evt = new MatchingTaskChangeEvent(task,
				MatchingTaskChangeEvent.EventType.MATCHER_ALIGNMENTSET_UPDATED);
		fireEvent(evt);
	}

	public Node getNode(String localname, boolean fromSource,
			boolean fromClasses) {
	...
	/***********************************************************************************************
	 * ************ Multi-ontology Support. ****************** - Cosmin, Nov 11,
	 * 2009.
	 */
	...
	// Ontology and Matcher IDs must be unique, hence they both increment the
	// same variable when a new id is required
	public int getNextOntologyID() {
		return IDcounter++;
	}

	public int getNextMatcherID() {
		return IDcounter++;
	}
	...
	public Ontology getOntologyByIndex(int index) { // TODO: should check for
													// and throw an
													// IndexOutOfBounds
													// exception
	...
	public synchronized void addOntology(Ontology ont) {
		if (!loadedOntologies.contains(ont)) {
			...
			// We must notify all the listeners that an ontology was loaded into
			// the system.
			OntologyChangeEvent e = new OntologyChangeEvent(this,
					OntologyChangeEvent.EventType.ONTOLOGY_ADDED, ont.getID());
			fireEvent(e);
		}
	}
	...
	public void removeOntology(Ontology ont) {
		if (loadedOntologies.contains(ont)) {
			...
			if (sourceOntology == ont)
				sourceOntology = null;
			if (targetOntology == ont)
				targetOntology = null;
			// Notify all the listeners that an ontology was removed from the
			// system
			OntologyChangeEvent e = new OntologyChangeEvent(this,
					OntologyChangeEvent.EventType.ONTOLOGY_REMOVED, ont.getID());
			fireEvent(e);
		}

		if (!ontologiesLoaded()) {
			currentProfiler = null;
		} // clear the ontology profiler
	}

	/********************************************************************************************
	 * Ontology Change Events fireEvent will send the event to all the
	 * listeners. The listeners get updated whenever a new ontology is loaded to
	 * the core, or an ontology is removed from the core.
	 */
	public void addOntologyChangeListener(OntologyChangeListener l) {
		ontologyListeners.add(l);
	}

	public void removeOntologyChangeListener(OntologyChangeListener l) {
		ontologyListeners.remove(l);
	}

	public void fireEvent(final OntologyChangeEvent event) {
		for (int i = ontologyListeners.size() - 1; i >= 0; i--) { // count DOWN
																	// from max
																	// (for a
																	// very good
																	// reason,
																	// http://book.javanb.com/swing-hacks/swinghacks-chp-12-sect-8.html
																	// )
			// execute each event in its own thread.

			final int finalI = i; // declared final so that it may be used in
									// the anonymous Thread class.
			Thread t = new Thread() {
				public void run() {
	...
	public void addMatcherChangeListener(MatcherChangeListener l) {
		matcherListeners.add(l);
	}

	public void removeMatcherChangeListener(MatcherChangeListener l) {
		matcherListeners.remove(l);
	}

	public void fireEvent(final MatchingTaskChangeEvent event) {
		for (int i = matcherListeners.size() - 1; i >= 0; i--) { // count DOWN
																	// from max
																	// (for a
																	// very good
																	// reason,
	...
	/**
	 * Ontology profiler. Set the profiler by using the Ontology -> Profiling...
	 * menu.
	 * 
	 * @return The current ontology profiling algorithm set.
	 */
	public OntologyProfiler getOntologyProfiler() {
	 ...
	 * @param p
	 */
	public void setOntologyProfiler(OntologyProfiler p) {
		currentProfiler = p;
	}

	public void initializeOSGiRegistry(BundleContext context)
			throws InstanceAlreadyExistsException {
		this.bundleContext = context;
		if (registry != null)
			throw new InstanceAlreadyExistsException(
					"You've already instantiated an OSGiRegistry object.");
	...
	/**
	 * Gracefully shutdown AgreementMaker. This was introduced mainly to handle
	 * OSGi shutdowns.
	 */
