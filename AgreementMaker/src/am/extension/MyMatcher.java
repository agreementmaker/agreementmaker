...
//        double dividedSynonymSimilarity = findDividedSynonymSimilarity(source, target);
    ...
    /*
     * dividedSynonymSimilarity was an initiative to find the substrings of camelCaseWords, and 
     * check the individual similarity. 
     * Due to the high level of complexity, and lack of significant results, this was dropped from the main algorithm. 
     * 
     */

    private double findDividedSynonymSimilarity(Node source, Node target) {

        String[] inputLabelWords = source.getLabel().replaceAll("\\s", "")
                .split("(?<!(^|[A-Z0-9]))(?=[A-Z0-9])|(?<!(^|[^A-Z]))(?=[0-9])|(?<!(^|[^0-9]))(?=[A-Za-z])|(?<!^)(?=[A-Z][a-z])");
        String[] targetLabelWords = target.getLabel().replaceAll("\\s", "")
                .split("(?<!(^|[A-Z0-9]))(?=[A-Z0-9])|(?<!(^|[^A-Z]))(?=[0-9])|(?<!(^|[^0-9]))(?=[A-Za-z])|(?<!^)(?=[A-Z][a-z])");
        String[] inputNameWords = source.getLocalName().replaceAll("\\s", "")
                .split("(?<!(^|[A-Z0-9]))(?=[A-Z0-9])|(?<!(^|[^A-Z]))(?=[0-9])|(?<!(^|[^0-9]))(?=[A-Za-z])|(?<!^)(?=[A-Z][a-z])");
        String[] targetNameWords = target.getLocalName().replaceAll("\\s", "")
                .split("(?<!(^|[A-Z0-9]))(?=[A-Z0-9])|(?<!(^|[^A-Z]))(?=[0-9])|(?<!(^|[^0-9]))(?=[A-Za-z])|(?<!^)(?=[A-Z][a-z])");

        FromWordNetUtils wordNetUtils = new FromWordNetUtils();

        double camelLabelLabelRanking = 0;
        for (String inputSubString : inputLabelWords) {
            for (String targetSubString : targetLabelWords) {
                // System.out.println(targetSubString);
                if (inputSubString.equals(targetSubString) || (wordNetUtils.areSynonyms(inputSubString, targetSubString))) {
                    camelLabelLabelRanking = camelLabelLabelRanking + 1 / inputLabelWords.length;
                }
            }
        }

        double camelNameNameRanking = 0;
        for (String inputSubString : inputNameWords) {
            for (String targetSubString : targetNameWords) {
                // System.out.println(targetSubString);
                if (inputSubString.equals(targetSubString) || (wordNetUtils.areSynonyms(inputSubString, targetSubString))) {
                    camelNameNameRanking = camelNameNameRanking + 1 / inputLabelWords.length;
                }
            }
        }

        double camelLabeNamelRanking = 0;
        for (String inputSubString : inputLabelWords) {
            for (String targetSubString : targetNameWords) {
                // System.out.println(targetSubString);
                if (inputSubString.equals(targetSubString) || (wordNetUtils.areSynonyms(inputSubString, targetSubString))) {
                    camelLabeNamelRanking = camelLabeNamelRanking + 1 / inputLabelWords.length;
                }
            }
        }

        double camelNameLabelRanking = 0;
        for (String inputSubString : inputNameWords) {
            for (String targetSubString : targetLabelWords) {
                // System.out.println(targetSubString);
                if (inputSubString.equals(targetSubString) || (wordNetUtils.areSynonyms(inputSubString, targetSubString))) {
                    camelNameLabelRanking = camelNameLabelRanking + 1 / inputLabelWords.length;
                }
            }
        }
        Comparator comparator = Collections.reverseOrder();
        List<Double> list = new ArrayList<Double>();
        list.add(camelLabelLabelRanking);
        list.add(camelNameLabelRanking);
        list.add(camelLabeNamelRanking);
        list.add(camelNameNameRanking);
