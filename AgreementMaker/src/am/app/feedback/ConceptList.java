...
public class ConceptList extends ArrayList<CandidateConcept> {
	...
	public double getRelevance( Node n, int ontType, alignType t ) {
		
		Iterator<CandidateConcept> i = iterator();
		while( i.hasNext() ) {
			CandidateConcept cc = i.next();
			if( cc.isFromOntology(ontType) && cc.isType(t) && cc.getNode().equals(n) ) {
				// this is a match
				//System.out.println( "++ " + cc.toString() + "==" + n.toString() ); 
